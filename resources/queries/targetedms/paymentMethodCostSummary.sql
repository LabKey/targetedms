SELECT
    ...
    (SUM(iup.percentPayment * rt.setupFee) / 100) AS SetupFee,
    timestampdiff('SQL_TSI_HOUR', isc.startTime, isc.endTime) * (SUM(iup.percentPayment * ir.fee) / 100) AS InstrumentCost,
    timestampdiff('SQL_TSI_HOUR', isc.startTime, isc.endTime) * (SUM(iup.percentPayment * ir.fee) / 100) + (SUM(iup.percentPayment * rt.setupFee) / 100) AS TotalCost
...
LEFT JOIN instrumentRate ir ON isc.instrument = ir.instrument
